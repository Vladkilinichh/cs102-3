...
class GameOfLife:
    ...
    def draw_grid(self) -> None:
        ...
        for i in range(self.cell_height):
            for j in range(self.cell_width):
                color = "green" if self.grid[i][j] else "white"
                a = self.cell_size
    ...
    def get_neighbours(self, cell: Cell) -> Cells:
        ...
        """
        x, y = cell
        possible_coord = [(x - 1, y + 1), (x, y + 1), (x + 1, y + 1),
                          (x - 1, y),                 (x + 1, y),
        ...
        for x, y in possible_coord:
            if (0 <= x < self.cell_width) and (0 <= y < self.cell_height):
                Cells.append((x, y))
        possible_coord = Cells
        return [self.grid[y1 % self.cell_height][x1 % self.cell_width] for x1, y1 in possible_coord]
    ...
    def get_next_generation(self) -> Grid:
        ...
        for y in range(self.cell_height):
            for x in range(self.cell_width):
                neighbours_c = self.get_neighbours((y, x)).count(1)
                if self.grid[y][x] == 0:
                    if neighbours_c == 3:
                        new_grid[y][x] = 1
                else:
                    if neighbours_c in [2, 3]:
                        new_grid[y][x] = 1
        return new_grid
